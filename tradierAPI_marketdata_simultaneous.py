...
def convert_numpy_types(data):
    ...
    if isinstance(data, dict):
        ...
        return data
async def get_multiple_options_data(db_session, session, tickers, loop_start_time):
    headers = {"Authorization": f"Bearer {real_auth}", "Accept": "application/json"}

    # Fetch quotes for all tickers at once
    ticker_quotes = await fetch(
        session,
    ...
    for ticker in tickers:
        ...
        quote_df = pd.DataFrame([quote for quote in ticker_quotes["quotes"]["quote"] if quote["symbol"] == ticker])
        ...
        # Prepare SymbolQuote data
        stock_price_data = {
            'symbol_name': symbol_name,
            'fetch_timestamp': loop_start_time,
            'trade_date': datetime.fromtimestamp(quote_df.at[0, "trade_date"] / 1000.0, tz=eastern),
            'last_price': quote_df.at[0, "last"],
            'bid': quote_df.at[0, "bid"],
            'bidsize': quote_df.at[0, "bidsize"],
            'bidexch': quote_df.at[0, "bidexch"],
            'bid_date': datetime.fromtimestamp(quote_df.at[0, "bid_date"] / 1000.0, tz=eastern) if not pd.isnull(
                quote_df.at[0, "bid_date"]) else None,
            'ask': quote_df.at[0, "ask"],
            'asksize': quote_df.at[0, "asksize"],
            'askexch': quote_df.at[0, "askexch"],
            'ask_date': datetime.fromtimestamp(quote_df.at[0, "ask_date"] / 1000.0, tz=eastern) if not pd.isnull(
            ...
            'prevclose': quote_df.at[0, "prevclose"],
            'change': quote_df.at[0, "change"],
            'change_percentage': quote_df.at[0, "change_percentage"]
        }
        all_stock_price_data.append(stock_price_data)

        # Fetch and process option data
        contract_quotes = await post_market_quotes(session, ticker, real_auth)

        if contract_quotes is not None:
            options_df = process_option_quotes(contract_quotes, current_price, prevclose)
            ...
            all_option_data.append(option_data_df)
            ...
            all_option_quotes.extend(option_quotes_data)
        ...
        if not ta_df.empty:
            ...
            all_ta_data.extend(ta_data_list)
        else:
            print(f"TA DataFrame is empty for {ticker}")

    # Bulk upsert for Symbol
    symbol_stmt = insert(Symbol).values(all_symbol_data)
    symbol_stmt = symbol_stmt.on_conflict_do_update(
        index_elements=[Symbol.symbol_name],
        set_={c.name: c for c in symbol_stmt.excluded if c.name != 'symbol_name'}
    )
    db_session.execute(symbol_stmt)

    # Convert numpy types to Python types
    all_stock_price_data = convert_numpy_types(all_stock_price_data)

    # Bulk insert for SymbolQuote
    db_session.execute(insert(SymbolQuote), all_stock_price_data)

    # Bulk upsert for Option
    if all_option_data:
        combined_option_data = pd.concat(all_option_data)
        upsert(
            con=engine,
            df=combined_option_data,
            table_name="options",
            schema="public",
    ...
    # Bulk insert for OptionQuote
    if all_option_quotes:
        all_option_quotes = convert_numpy_types(all_option_quotes)

        db_session.execute(insert(OptionQuote), all_option_quotes)

    # Bulk insert for TechnicalAnalysis
    if all_ta_data:
        all_ta_data = convert_numpy_types(all_ta_data)

        db_session.execute(insert(TechnicalAnalysis), all_ta_data)

    db_session.commit()

    return {ticker: (quote_df.at[0, "prevclose"], quote_df.at[0, "last"]) for ticker in tickers}


# Modify post_market_quotes to handle multiple tickers
async def post_market_quotes(session, tickers, real_auth):
    ...
    all_contracts = []
    for ticker in tickers:
        contracts = await lookup_all_option_contracts(session, ticker, real_auth)
        if contracts:
