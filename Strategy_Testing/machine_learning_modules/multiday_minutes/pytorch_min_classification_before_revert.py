...
def train_model(hparams, X, y):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_np)):
        ...
        for epoch in range(num_epochs):
            ...
            epoch_avg_precision = prec(torch.tensor(all_val_outputs).to(device),
...
try:
    study = optuna.load_study(study_name='SPY_FFNNClassificationCV_avg_val_per_epoch_across_folds', storage='sqlite:///SPY_FFNNClassificationCV_avg_val_per_epoch_across_folds.db')
    print("Study Loaded.")
except KeyError:
    study = optuna.create_study(direction="minimize", study_name='SPY_FFNNClassificationCV_avg_val_per_epoch_across_folds',
