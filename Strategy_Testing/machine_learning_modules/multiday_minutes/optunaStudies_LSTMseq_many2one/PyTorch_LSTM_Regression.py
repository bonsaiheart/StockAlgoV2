...
class LSTMRegressionNNSequence(nn.Module):
    def __init__(self, input_dim, num_hidden_units, dropout_rate, num_layers, sequence_length):
...
def train_model(hparams, X, y_change, trial=None):
    ...
    for fold, (train_index, val_index) in enumerate(tscv.split(X)):
        ...
        model = LSTMRegressionNNSequence(X.shape[1], hidden_size,
                                         dropout_rate, num_layers, hparams["sequence_length"]).to(
            device)
        ...
        for epoch in range(num_epochs):
            ...
            for i, (X_val_batch, y_val_batch) in enumerate(val_loader):
                if X_val_batch.size(0) != batch_size:
                    ...
                    h_0 = torch.zeros(num_layers, X_val_batch.size(0), hidden_size).to(device)
                    c_0 = torch.zeros(num_layers, X_val_batch.size(0), hidden_size).to(device)
                else:
                ...
                with torch.no_grad():
                    val_outputs = model(X_val_batch, (h_0, c_0))  # Pass hidden state if your model requires it
...
def objective(trial):
    ...
    print("best mae score: ", bestmodel_avg_mae_score, "Inverse Scaled MAE: ", inverse_scaled_mae," best mse score: ", bestmodel_avg_mse_score,
          "smallest val loss: ",
...
model_up_nn = LSTMRegressionNNSequence(X.shape[1], best_params["num_hidden_units"],
                                       best_params.get('dropout_rate', 0)
                                       , best_params["num_layers"],
                                       best_params["sequence_length"]).to(device)
