...
Chosen_Predictor = ['Bonsai Ratio','Bonsai Ratio 2','B1/B2','B2/B1','PCRoi Up1','PCRoi Down1','ITM PCR-OI','ITM PCRoi Up1','ITM PCRoi Down1','ITM PCRoi Down2','ITM PCRoi Down3','ITM PCRoi Down4','ITM Contracts %','Net ITM IV','NIV highers(-)lowers1-4','Signal_Line','CCI','CMF','EoM','OBV','MFI','Keltner_Upper','Keltner_Lower','VPT']
...
lastxminutes = 60*6.5*5*8 #2 monht
ml_dataframe = ml_dataframe[-int(lastxminutes):]
...
threshold = 1e10
for feature in Chosen_Predictor:
    X[feature] = np.clip(X[feature], -threshold, threshold)
...
def train_model(hparams, X_train, y_train, X_val, y_val):
    ...
    best_valloss_score = np.inf # Track the best F1 score
    ...
    # Validation step
        with torch.no_grad():
            ...
            if val_loss<best_valloss_score:
                best_model_state_dict = model.state_dict()
                best_valloss_score = val_loss
        ...
        # print( f"VALIDA??TION Epoch: {epoch + 1}, PrecisionScore: {PrecisionScore.item()}, Training Loss: {loss.item()}, Validation Loss: {val_loss.item()}, F1 Score: {F1Score.item()} ")
        if counter >= patience:
            try:
    ...
    return best_valloss_score, best_f1_score,best_prec_score,best_model_state_dict # Return the best F1 score after all epochs
# Define Optuna Objective
def objective(trial):
    # Define the hyperparameter search space
    learning_rate = trial.suggest_float("learning_rate",  1e-7,0.01,log=True)#0003034075497582067
    num_epochs = trial.suggest_int("num_epochs", 10000, 10005)#3800 #230  291
    ...
    num_hidden_units = trial.suggest_int("num_hidden_units", 32, 2500)#2560 #83 125 63


    # Call the train_model function with the current hyperparameters
    bestlast_val_loss,f1_score, prec_score,best_model_state_dict = train_model(
        {
...
study.optimize(objective, n_trials=0)  # You can change the number of trials as needed
...
(bestlast_valloss,best_f1_score,best_prec_score,best_model_state_dict) = train_model(
...
task = "binary"
precision_up = Precision(num_classes=2, average='macro', task='binary').to(device)(predicted_up_tensor,y_up_test_tensor )  # move metric to same device as tensors
