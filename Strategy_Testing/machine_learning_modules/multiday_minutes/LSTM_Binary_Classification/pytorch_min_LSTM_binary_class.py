...
class BinaryClassificationLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim,dropout_rate, num_layers,sequence_length ):
        ...
        self.sequence_length = sequence_length  # Add the sequence length as an attribute
    ...
    def forward(self, x):
        ...
        out = self.output_layer(out[:, -1, :])  # Removed the sigmoid activation here for BCEwithlogitsloss.
...
def train_model(hparams, X, y):
    ...
    for fold, (train_index, val_index) in enumerate(tscv.split(X_np)):
        ...
        X_train_sequences = to_sequences(X_train_tensor, sequence_length)  # Convert to sequences

        X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32).to(device)
        X_val_sequences = to_sequences(X_val_tensor, sequence_length)  # Convert to sequences
...
def objective(trial):
    ...
    num_hidden_units = trial.suggest_int("num_hidden_units", 100, 2000)  # 2560 #83 125 63  #7000
...
test_result= train_model(
    best_params, X, y)
model_up_nn = BinaryClassificationLSTM(X.shape[1], best_params["num_hidden_units"],
                                          best_params["dropout_rate"], best_params["sequence_length"]).to(device)
...
print(f"Test Number of positive predictions: {sum(x[0] for x in predicted_probabilities)}")
