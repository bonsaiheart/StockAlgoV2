...
class BinaryClassificationLSTM(nn.Module):
    def __init__(
        self, input_dim, hidden_dim, dropout_rate, num_layers, sequence_length
    ):
    ...
    def forward(self, x):
        h_0 = torch.zeros(self.lstm.num_layers, x.size(0), self.lstm.hidden_size).to(
            x.device
        )
        c_0 = torch.zeros(self.lstm.num_layers, x.size(0), self.lstm.hidden_size).to(
            x.device
        )
        ...
        out = self.output_layer(
            out[:, -1, :]
        )  # Removed the sigmoid activation here for BCEwithlogitsloss.
        ...
        for epoch in range(num_epochs):
            ...
            for X_batch, y_batch in train_loader:
                if optimizer_name == "LBFGS":
                    ...
                    def closure():
                        ...
                        outputs = outputs.squeeze()  # Squeeze the extra dimension
                        loss = criterion(outputs, y_batch)
                        # customloss = specialcriterion(outputs, y_batch)
            ...
            for X_val_batch, y_val_batch in val_loader:
                with torch.no_grad():
                    ...
                    val_outputs = val_outputs.squeeze()  # Squeeze the extra dimension
                    val_probs = torch.sigmoid(val_outputs)

                    all_val_outputs.extend(val_probs.tolist())
            ...
            preds_tensor = torch.tensor(all_val_outputs_bin)
...
def objective(trial):
    ...
    batch_size = trial.suggest_int("batch_size", 20, 200)  # 2000
    num_hidden_units = trial.suggest_int(
        "num_hidden_units", 100, 2000
    )  # 2560 #83 125 63  #7000
    ...
    num_layers = trial.suggest_int("num_layers", 1, 3)  # example
    if num_layers == 1:
        dropout_rate = 0
    else:
...
try:
    study = optuna.load_study(
        study_name="SPY_FFNNClassificationCV_best_microf1",
...
model_up_nn = BinaryClassificationLSTM(
    X.shape[1],
...
task = "binary"
precision_up = Precision(num_classes=2, average="micro", task="binary").to(device)(
...
# print(f"Precision for class 1: {precision_for_class_1}")
accuracy_up = Accuracy(num_classes=2, average="micro", task=task).to(device)(
    predicted_up_tensor, y_test_tensor
)
recall_up = Recall(num_classes=2, average="micro", task=task).to(device)(
    predicted_up_tensor, y_test_tensor
)
f1_up = F1Score(num_classes=2, average="micro", task=task).to(device)(
    predicted_up_tensor, y_test_tensor
)
