...
import calculations
import trade_algos2
import tradierAPI_marketdata_simultaneous
    ...
    # while start_time <= market_close_time_utc:
        ...
        try:
            ...
            with Session(engine) as session:  # Use the Session object directly to create a session
                ...
                option_data_success = await tradierAPI_marketdata_simultaneous.get_multiple_options_data(
...
async def main():
    try:
        ...
        tradierAPI_marketdata_simultaneous.create_database_tables(engine)
        ...
        # for ticker in tickerlist:
        task = asyncio.create_task(handle_ticker_cycle(session, tickerlist))
