...
class IBOrderManager:
    ...
    def on_order_status_change(self, trade, order_status):
        ...
        if order_status.status in ['Filled', 'Cancelled', 'Inactive']:
            ...
            if order_id in self.order_events:
                self.order_events[order_id].set()  # Signal that the order is done
        # Unsubscribe to avoid unnecessary calls
    ...
    async def ib_connect(self):
        if not self.ib.isConnected():
            print("~~~ Connecting ~~~")
            try:
                await self.ib.connectAsync("192.168.1.119", 7497, clientId=0, timeout=45)
    ...
    async def getTrade(self,order):
        trade = next((trade for trade in self.ib.trades() if trade.order is order), None)
    ...
    async def cancel_and_replace_orders(self,contract, action, CorP, ticker, exp, strike, contract_current_price, quantity,
        ...
        if not child_orders_objs_list:
            ...
            oca_group_remaining_qty = await self.cancel_oca_group_orders(child_orders_objs_list)
            order_details = {}
            ...
            parenttrade = await self.placeOptionBracketOrder(
                CorP,
                ticker,
                exp,
                strike,
    ...
    async def replace_child_orders(self,order_details, contract):
        try:
            for ocaGroup, child_order_details in order_details.items():
                ...
                while True:
                    stoplossTrade = await self.getTrade(stopLoss)
                    takeprofitTrade = await self.getTrade(takeProfit)
    ...
    async def placeOptionBracketOrder(self,
            CorP,
            ticker,
            exp,
            strike,
        ...
        if await self.can_place_new_order(ib, qualified_contract):
            if check_opposite_orders:
                ...
                try:
                    ...
                    #
                    #     # ib.sleep(1)
                    #     self.ib.placeOrder(ticker_contract, o)
                    ...
                    # Place the bracket order and register the event handler
                    for o in bracketOrder:
                        o.ocaType = 2

                        trade = self.ib.placeOrder(ticker_contract, o)
                        self.pending_orders[trade.order.orderId] = trade
                        self.order_events[trade.order.orderId] = asyncio.Event()

                        # Store the parent trade for return
                        if trade.order == parent:
                            parent_trade = trade

                    # Asynchronously wait for the orders to be done (filled, cancelled, or inactive)
                    await asyncio.gather(*(self.order_events[orderId].wait() for orderId in self.pending_orders))
