...
def wait_60_minutes_and_send_tweet(ticker,current_price,tweet_id,upordown):
    ...
    if upordown == "up":
        reply_tweet_1_hour_later(
            f"As of {highest_price_time} EST, ${ticker} was ${highest_price}. Up ${round(old_price - highest_price,3)}, or %{round(((old_price - highest_price) / highest_price) * 100,3)} from entry.",
            tweet_id)
    if upordown == "down":
        reply_tweet_1_hour_later(
